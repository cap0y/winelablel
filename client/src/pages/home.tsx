...
import { Wine, Palette, ShoppingCart, Star, Heart, Sparkles, LucideWine, ChevronLeft, ChevronRight } from "lucide-react";
...
function LabelSlider() {
  const [labels, setLabels] = useState<Array<{id: string, title: string, labelImage: string}>>([]);
  ...
  useEffect(() => {
    const fetchPopularLabels = async () => {
      try {
        ...
        let labelData: Array<{id: string, title: string, labelImage: string}> = [];
        ...
        // API 응답에서 라벨 가져오기
        if (response.data.success && response.data.labels && response.data.labels.length > 0) {
          console.log("API에서 가져온 라벨 수:", response.data.labels.length);
          labelData = response.data.labels;
          
          // 라벨이 3개 미만이면 기본 이미지로 보충
          if (labelData.length < 3) {
            console.log(`라벨이 ${labelData.length}개뿐이므로 기본 이미지로 보충합니다.`);
            ...
            if (labelData.length < 3) {
              console.error("보충 후에도 이미지가 3개가 안 됨. 강제로 3개 설정");
...
// 와인 디자인 과정 단계 컴포넌트
function DesignProcessCard({ icon, title, description }: { icon: React.ReactNode, title: string, description: string }) {
  return (
...
// 인기 와인 라벨 디자인 컴포넌트
function PopularDesignCard({ image, title, rating }: { image: string, title: string, rating: number }) {
  return (
...
export default function Home() {
  ...
  const popularDesigns = [
    { id: 1, image: "images/label/file-1753529313162-625550530.png", title: "클래식 레드", rating: 4.9 },
    { id: 2, image: "images/label/file-1753529415175-217795439.png", title: "모던 골드", rating: 4.8 },
    { id: 3, image: "images/label/file-1753530313930-858584954.png", title: "빈티지 브라운", rating: 4.7 },
    { id: 4, image: "images/label/file-1753530768142-739119583.png", title: "우아한 크림", rating: 4.6 }
  ];
    ...
    <div className="min-h-screen bg-gray-900 text-white">
      ...
      <section className="px-4 py-6">
        <div className="bg-gradient-to-r from-primary/20 to-primary/10 rounded-2xl p-6 mb-6">
          ...
          <p className="text-gray-300 mb-4">특별한 순간을 위한 특별한 와인 라벨을 만들어보세요</p>
          ...
          <div className="flex flex-col space-y-2">
            ...
            <Link href="/gallery">
              <Button variant="outline" className="w-full bg-gray-800/80 text-primary border-primary/30 hover:bg-gray-800/90 py-3 rounded-lg font-medium flex items-center justify-center">
      ...
      <section className="px-4 py-6">
        <Tabs defaultValue="process" className="w-full">
          <TabsList className="grid w-full grid-cols-3 bg-gray-800 border-gray-700">
            <TabsTrigger value="process" className="text-sm flex items-center gap-1 data-[state=active]:bg-primary data-[state=active]:text-white">
              <Palette className="w-4 h-4" />
              제작 과정
            </TabsTrigger>
            <TabsTrigger value="popular" className="text-sm flex items-center gap-1 data-[state=active]:bg-primary data-[state=active]:text-white">
