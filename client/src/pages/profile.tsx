...
export default function ProfilePage() {
  ...
  const handleChangePassword = async () => {
    ...
    if (!currentPassword || !newPassword || !confirmPassword) {
      toast({ title: "입력 필요", description: "모든 비밀번호 입력 칸을 채워주세요.", variant: "destructive" });
      return;
    }
    if (newPassword.length < 6) {
      toast({ title: "유효성 오류", description: "새 비밀번호는 최소 6자 이상이어야 합니다.", variant: "destructive" });
      return;
    }
    if (newPassword !== confirmPassword) {
      toast({ title: "유효성 오류", description: "새 비밀번호와 확인 비밀번호가 일치하지 않습니다.", variant: "destructive" });
      return;
    }
    if (newPassword === currentPassword) {
      toast({ title: "유효성 오류", description: "새 비밀번호는 현재 비밀번호와 달라야 합니다.", variant: "destructive" });
      return;
    }
    try {
      setIsChangingPassword(true);
      await userApi.changePassword(user.email, currentPassword, newPassword);
      toast({ title: "완료", description: "비밀번호가 변경되었습니다." });
      setIsPwDialogOpen(false);
      resetPasswordForm();
    } catch (e: any) {
      const message = e?.response?.data?.message || e?.message || "비밀번호 변경 중 오류가 발생했습니다.";
  ...
  const fetchUserOrders = async () => {
    try {
      ...
      const ordersWithPaymentStatus = orders.map((order: any) => ({
        ...order,
        paymentStatus: order.paymentStatus || getPaymentStatusFromOrderStatus(order.status || '결제대기')
      }));
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!allowedTypes.includes(file.type)) {
      setUploadError('지원하지 않는 파일 형식입니다. JPEG, PNG, GIF, WEBP 형식만 업로드 가능합니다.');
      setIsUploading(false);
      if (fileInputRef.current) fileInputRef.current.value = '';
      return;
    }
  ...
  // 주문 상태 업데이트
  const updateOrderStatus = async (orderId: string, status: 'pending' | 'processed' | 'completed' | 'cancelled' | '결제대기' | '결제완료' | '제작중' | '배송준비' | '배송중' | '배송완료' | '주문취소') => {
    try {
      ...
      if (!data.success) {
        toast({
          title: "상태 업데이트 실패",
          description: data.message || "주문 상태를 업데이트하는데 실패했습니다.",
          variant: "destructive",
        });
        return;
      }
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (data?.success) {
          setBankConfig({ bankName: data.bankName, accountNo: data.accountNo, accountHolder: data.accountHolder });
        }
      } catch {
        setBankConfig(null);
      }
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="text-sm space-y-1">
                  ...
                  {order.paymentId && (
                    <p><span className="font-medium">결제ID:</span> {order.paymentId}</p>
                  )}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="text-sm space-y-1 bg-gray-800/50 p-3 rounded-lg border border-gray-700">
                  ...
                  <p>
                    ...
                    <span className="text-gray-200">{(order.deliveryFee || 3000).toLocaleString()}원</span>
                  </p>
                  <p>
                    <span className="font-medium text-gray-300">총 결제 금액:</span> 
                    <span className="text-green-400 font-semibold">{(order.amount + (order.deliveryFee || 3000)).toLocaleString()}원</span>
                  </p>
                  {order.paymentId && (
                    <p>
                  ...
                  <p>
                    ...
                    <span className={`${
                      order.paymentId || order.status === '결제완료' || order.status === '제작중' || order.status === '배송준비' || order.status === '배송중' || order.status === '배송완료'
                        ? 'text-green-400' 
                  ...
                  <p>
                    <span className="font-medium text-gray-300">주문 상태:</span> 
                    <span className={`${
                      ...
                      {order.status === 'completed' || order.status === '배송완료'
                        ? '배송완료' 
                        : order.status === 'processed' || order.status === '제작중'
                        ? '제작중' 
                        : order.status === '배송준비'
                        ? '배송준비'
                        : order.status === '배송중'
              ...
              <div>
                ...
                <div className="text-sm space-y-1 bg-gray-800/50 p-3 rounded-lg border border-gray-700">
                  <p><span className="font-medium text-gray-300">이름:</span> <span className="text-gray-200">{order.customerName}</span></p>
                  <p><span className="font-medium text-gray-300">이메일:</span> <span className="text-gray-200">{order.customerEmail}</span></p>
                  {order.customerPhone && (
                    <p><span className="font-medium text-gray-300">전화번호:</span> <span className="text-gray-200">{order.customerPhone}</span></p>
                  )}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="text-sm space-y-1 bg-gray-800/50 p-3 rounded-lg border border-gray-700">
                  {order.customerAddress ? (
                    <>
                      <p><span className="font-medium text-gray-300">배송 주소:</span> <span className="text-gray-200">{order.customerAddress}</span></p>
                      {order.customerZipCode && (
                        <p><span className="font-medium text-gray-300">우편번호:</span> <span className="text-gray-200">{order.customerZipCode}</span></p>
                      )}
                    </>
                  ) : (
                      ...
                      <p>
                        ...
                        <span className="text-gray-200">
                          {order.shippingCompany === 'cj' ? 'CJ대한통운' : 
                           ...
                           order.shippingCompany === 'logen' ? '로젠택배' :
                           order.shippingCompany === 'post' ? '우체국택배' : '택배사'}
                        </span>
                      </p>
                      <p>
                        <span className="font-medium text-gray-300">운송장번호:</span> 
                        <span className="text-gray-200">{order.trackingNumber}</span>
                        <a 
      ...
      <Card className="bg-gray-800 border-gray-700 mb-6">
        ...
        <CardContent className="space-y-4">
          <div className="flex items-center justify-between gap-4">
            ...
            <Dialog open={isPwDialogOpen} onOpenChange={(o) => { setIsPwDialogOpen(o); if (!o) resetPasswordForm(); }}>
              ...
              <DialogContent className="max-w-md">
                ...
                <div className="space-y-4 pt-2">
                  <div>
                    ...
                    <Input id="current-password" type="password" value={currentPassword} onChange={(e) => setCurrentPassword(e.target.value)} placeholder="현재 비밀번호" />
                  </div>
                  <div>
                    <Label htmlFor="new-password">새 비밀번호</Label>
                    <Input id="new-password" type="password" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} placeholder="새 비밀번호 (6자 이상)" />
                  </div>
                  <div>
                    <Label htmlFor="confirm-password">새 비밀번호 확인</Label>
                    <Input id="confirm-password" type="password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} placeholder="새 비밀번호 확인" />
                  </div>
                  <div className="flex justify-end gap-2 pt-2">
                    <Button variant="outline" onClick={() => { setIsPwDialogOpen(false); resetPasswordForm(); }} disabled={isChangingPassword}>취소</Button>
        ...
        <div className="mt-8">
          ...
          <Tabs value={userActiveTab} onValueChange={setUserActiveTab as any}>
            <TabsList className="grid grid-cols-3 gap-2">
              ...
              <TabsTrigger value="stats">주문 통계</TabsTrigger>
            </TabsList>
                              ...
                              <td className="px-4 py-3">
                                <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs 
                                  ...
                                  {order.paymentStatus || (order.paymentId || order.status === '결제완료' || order.status === '제작중' || order.status === '배송준비' || order.status === '배송중' || order.status === '배송완료'
                                    ? '결제완료' 
                                    : order.status === '주문취소' || order.status === 'cancelled'
                                    ? '결제취소'
                                    : '결제대기')}
                                </span>
                                {/* 은행 결제 정보 추가 표시 */}
                                    ...
                                    {order.trackingNumber && (
                                      <div className="text-cyan-400 text-xs mt-1">
                                        {order.shippingCompany === 'cj' ? 'CJ대한통운' : 
                                         ...
                                         order.shippingCompany === 'hanjin' ? '한진택배' :
                                         order.shippingCompany === 'logen' ? '로젠택배' :
                                         order.shippingCompany === 'post' ? '우체국택배' : '택배사'}: {order.trackingNumber}
                                      </div>
                                    )}
                                  </div>
                                ) : (
                        ...
                        >
                          <div className="flex items-start justify-between">
                            ...
                            <div className="flex flex-col space-y-2">
                              {notification.orderId && typeof notification.orderId === 'string' && (
                                <Button
                                  variant="outline"
                                  size="sm"
                                  className="text-blue-400 hover:text-blue-300"
