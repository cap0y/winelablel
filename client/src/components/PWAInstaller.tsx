...
export default function PWAInstaller() {
  ...
  useEffect(() => {
    ...
    // 무조건 홈 화면에 설치 배너 노출: 설치 안된 경우 즉시 표시
    if (!window.matchMedia('(display-mode: standalone)').matches) {
  ...
  // 설치 안내 배너 표시 조건: 설치되지 않았고 표시 플래그가 true면 항상 표시
  ...
  const platformInstructions = () => {
    ...
    if (isIOS) {
      return 'iOS: Safari에서 공유 버튼(위 화살표) > 홈 화면에 추가를 선택하세요.';
    }
    if (isAndroid) {
      return 'Android: 브라우저 메뉴(⋮) > 홈 화면에 추가를 선택하세요.';
    }
    if (isSafari) {
      return 'Safari: 공유 버튼 > 홈 화면에 추가를 선택하세요.';
    }
        ...
        <div className="fixed bottom-4 left-4 right-4 z-50 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg p-4">
          <div className="flex items-center justify-between">
            <div className="flex-1">
              ...
              {showInstructions && (
                <p className="text-xs text-blue-400 mt-2">{platformInstructions()}</p>
              )}
            </div>
            <div className="flex items-center gap-2 ml-4">
              ...
              <Button
                size="sm"
                variant="ghost"
                onClick={() => { setShowInstallPrompt(false); setShowInstructions(false); }}
              >
                <X className="w-4 h-4" />
              </Button>
            </div>
