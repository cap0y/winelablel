...
const getStatusBadgeColor = (status: OrderStatus) => {
  switch (status) {
    case '결제대기': return 'bg-yellow-500/20 text-yellow-400 border border-yellow-500';
    case '결제완료': return 'bg-blue-500/20 text-blue-400 border border-blue-500';
    case '제작중': return 'bg-purple-500/20 text-purple-400 border border-purple-500';
    case '배송준비': return 'bg-indigo-500/20 text-indigo-400 border border-indigo-500';
    case '배송중': return 'bg-cyan-500/20 text-cyan-400 border border-cyan-500';
    case '배송완료': return 'bg-green-500/20 text-green-400 border border-green-500';
    case '주문취소': return 'bg-red-500/20 text-red-400 border border-red-500';
...
const OrderManagement = () => {
  ...
  const [shippingCompany, setShippingCompany] = useState<Record<string, string>>({});
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.success) {
          ...
          setAdminUnreadCount(data.notifications?.filter((n: AdminNotification) => !n.isRead).length || 0);
        }
      }
    } catch (error) {
  ...
  const fetchOrders = useCallback(async (forceRefresh = false) => {
    ...
    if (!forceRefresh && orders.length > 0 && (now - lastFetchTime) < cacheExpiry) {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        const ordersWithPaymentStatus = fetchedOrders.map((order: any) => ({
          ...order,
          paymentStatus: getPaymentStatusFromOrderStatus(order.status || '결제완료')
        }));
  ...
  useEffect(() => {
    if (orders.length > 0) {
      const updatedOrders = orders.map(order => ({
        ...order,
        paymentStatus: order.paymentStatus || getPaymentStatusFromOrderStatus(order.status || '결제완료')
      }));
      
      // 결제 상태가 업데이트된 주문이 있는지 확인
      const hasUpdates = updatedOrders.some((order, index) => 
        order.paymentStatus !== orders[index].paymentStatus
      );
  ...
  // 주문 상태에 따른 결제 상태 결정
  const getPaymentStatusFromOrderStatus = (orderStatus: OrderStatus | string) => {
    // 문자열로 들어오는 경우도 처리
  ...
  const handleSendShippingNotification = async (orderId: string) => {
    try {
      ...
      // 배송사 이름 가져오기
      const shippingCompanyName = shippingCompanies.find(c => c.id === order.shippingCompany)?.name || '택배사';
  ...
  const handleViewDetails = async (orderId: string) => {
    try {
      ...
      if (data.order.labelDesign && typeof data.order.labelDesign === 'string') {
        try {
          const parsedLabelDesign = JSON.parse(data.order.labelDesign);
          
          // 장식 정보에 이미지 URL이 없는 경우를 위한 추가 처리
          if (parsedLabelDesign.decorations && Array.isArray(parsedLabelDesign.decorations)) {
            ...
            try {
              ...
              // 장식 정보에 이미지 URL 추가
              parsedLabelDesign.decorations = parsedLabelDesign.decorations.map((deco: any) => {
                // 이미 이미지 URL이 있으면 그대로 사용
                if (deco.image) return deco;
                
                // API에서 가져온 장식 목록에서 찾기
                const matchingIcon = iconsData.find((icon: any) => icon.id === deco.decorationId);
                if (matchingIcon) {
                  return {
                    ...deco,
      ...
      // 장식 정보 있는지 확인 및 로그
      if (orderWithLabelDesign.labelDesign && orderWithLabelDesign.labelDesign.decorations) {
        console.log("장식 정보 로드:", orderWithLabelDesign.labelDesign.decorations);
      } else {
            ...
            <div class="wine-bottle-container">
              ...
              <div class="label-overlay">
                ...
                ` : `
                  ...
                  ${order.labelDesign?.decorations ? order.labelDesign.decorations.map((deco: any) => `
                    <img 
      ...
      <CardContent className="p-0">
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex items-start space-x-2 flex-1">
                        ...
                        <div className={`p-1 rounded-full ${
                          ...
                          {notification.type === 'shipping' && <Truck className="w-3 h-3" />}
                          {notification.type === 'order' && <Package className="w-3 h-3" />}
        ...
        {isLoading ? (
          <div className="flex flex-col justify-center items-center h-64 space-y-4">
            ...
            <p className="text-sm text-gray-500">데이터가 많을 경우 시간이 걸릴 수 있습니다.</p>
          </div>
        ) : (
          <div className="overflow-x-auto">
            ...
            <div className="p-4 bg-gray-800/30 border-b border-gray-700">
              <div className="flex justify-between items-center text-sm text-gray-400">
                ...
                <span>마지막 업데이트: {new Date(lastFetchTime).toLocaleTimeString()}</span>
              </div>
            </div>
            ...
            <Table className="border-collapse">
              <TableHeader className="bg-gray-800/50">
                <TableRow className="border-b border-gray-700">
                  <TableHead className="w-[100px] cursor-pointer text-gray-300 hover:text-cyan-400 transition-colors" onClick={() => handleSort('id')}>
                    고객이름 <ArrowUpDown className="inline w-4 h-4" />
                  </TableHead>
                  <TableHead className="cursor-pointer text-gray-300 hover:text-cyan-400 transition-colors" onClick={() => handleSort('bottleName')}>
                    주문상품 <ArrowUpDown className="inline w-4 h-4" />
                  </TableHead>
                  <TableHead className="cursor-pointer text-gray-300 hover:text-cyan-400 transition-colors" onClick={() => handleSort('amount')}>
                    결제금액 <ArrowUpDown className="inline w-4 h-4" />
                  </TableHead>
                  <TableHead className="text-gray-300">배송정보</TableHead>
                  <TableHead className="cursor-pointer text-gray-300 hover:text-cyan-400 transition-colors" onClick={() => handleSort('status')}>
                ...
                {filteredOrders.length === 0 ? (
                  <TableRow className="border-b border-gray-800 hover:bg-gray-800/50">
                    <TableCell colSpan={8} className="text-center py-10 text-gray-400">
                      {searchTerm ? '검색 조건에 맞는 주문이 없습니다.' : '주문 내역이 없습니다.'}
                    </TableCell>
                  </TableRow>
                ) : (
                  filteredOrders.map(order => (
                    <TableRow key={order.id} className="border-b border-gray-800 hover:bg-gray-800/50">
                      ...
                      <TableCell className="text-gray-300">
                        {new Intl.NumberFormat('ko-KR').format(order.amount || 0)}원/매
                      </TableCell>
                      <TableCell>
                        <div className="flex flex-col text-sm">
                          <span className="font-semibold text-gray-200">{order.customerName}</span>
                          <span className="text-gray-400">{order.customerPhone || order.phoneNumber}</span>
                          <span className="text-gray-400 truncate max-w-[200px]">
                            {order.customerAddress || order.address || '배송 주소 없음'}
                          </span>
                        </div>
                      </TableCell>
                      <TableCell>
                        <Badge className={getStatusBadgeColor(order.paymentStatus || getPaymentStatusFromOrderStatus(order.status || '결제완료'))}>
                          {order.paymentStatus || getPaymentStatusFromOrderStatus(order.status || '결제완료')}
                        </Badge>
                      </TableCell>
                      <TableCell>
                        <Select 
                        ...
                        >
                          ...
                          <SelectContent className="bg-gray-800 border-gray-700 text-gray-200">
                            <SelectItem value="결제대기" className="focus:bg-cyan-900/30 focus:text-cyan-400">결제대기</SelectItem>
                            <SelectItem value="결제완료" className="focus:bg-cyan-900/30 focus:text-cyan-400">결제완료</SelectItem>
                            <SelectItem value="제작중" className="focus:bg-cyan-900/30 focus:text-cyan-400">제작중</SelectItem>
                            <SelectItem value="배송준비" className="focus:bg-cyan-900/30 focus:text-cyan-400">배송준비</SelectItem>
                            <SelectItem value="배송중" className="focus:bg-cyan-900/30 focus:text-cyan-400">배송중</SelectItem>
                            <SelectItem value="배송완료" className="focus:bg-cyan-900/30 focus:text-cyan-400">배송완료</SelectItem>
                            <SelectItem value="주문취소" className="focus:bg-cyan-900/30 focus:text-cyan-400">주문취소</SelectItem>
                          </SelectContent>
                        </Select>
                      </TableCell>
                      <TableCell>
                          ...
                          <div className="flex flex-col space-y-2">
                            <div className="flex space-x-2">
                              <Select 
                                ...
                                onValueChange={(value) => handleShippingCompanyChange(order.id, value)}
                              >
                                ...
                                <SelectContent className="bg-gray-800 border-gray-700 text-gray-200">
                                  {shippingCompanies.map(company => (
                                    <SelectItem key={company.id} value={company.id} className="focus:bg-cyan-900/30 focus:text-cyan-400">
                                      {company.name}
                                    </SelectItem>
                                  ))}
                                </SelectContent>
                          ...
                          <div className="flex flex-col space-y-1">
                            {order.trackingNumber ? (
                              <>
                                <div className="flex items-center space-x-2">
                                  <span className="text-sm text-gray-300">
                                    {shippingCompanies.find(c => c.id === order.shippingCompany)?.name || '택배사'}: {order.trackingNumber}
                                  </span>
                                  <a 
                                    href={getTrackingUrl(order.shippingCompany || 'cj', order.trackingNumber)}
                                    target="_blank"
                              ...
                              <Button 
                                ...
                                className="bg-gray-800/50 hover:bg-gray-700 text-gray-300 border-gray-700"
                              >
                                <Truck className="w-4 h-4 mr-1 text-cyan-500" /> 운송장 입력
                              </Button>
                            )}
                          </div>
                        )}
                      ...
                      <TableCell>
                        <div className="flex flex-col space-y-2">
                          ...
                          <Button 
                            size="sm" 
                            variant="outline" 
                            onClick={() => handleSendShippingNotification(order.id)}
          ...
          {selectedOrder && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-4">
                <div>
                  <h3 className="text-lg font-medium mb-2 text-cyan-400">주문 정보</h3>
                  <div className="text-sm space-y-1 bg-gray-800/50 p-3 rounded-lg border border-gray-800">
                    <p><span className="font-medium text-gray-300">주문번호:</span> <span className="text-gray-200">{selectedOrder.id}</span></p>
                    <p><span className="font-medium text-gray-300">주문일자:</span> <span className="text-gray-200">{new Date(selectedOrder.createdAt).toLocaleDateString()}</span></p>
                    <p><span className="font-medium text-gray-300">주문상태:</span> <span className="text-gray-200">{selectedOrder.status}</span></p>
                    <p><span className="font-medium text-gray-300">와인병:</span> <span className="text-gray-200">{selectedOrder.bottleName}</span></p>
                    <p><span className="font-medium text-gray-300">수량:</span> <span className="text-gray-200">{selectedOrder.quantity || 1}개</span></p>
                    <p><span className="font-medium text-gray-300">상품 금액:</span> <span className="text-gray-200">{new Intl.NumberFormat('ko-KR').format(selectedOrder.amount)}원</span></p>
                    {selectedOrder.paymentId && (
                      <p><span className="font-medium text-gray-300">결제ID:</span> <span className="text-gray-200">{selectedOrder.paymentId}</span></p>
                    )}
                  </div>
                </div>
                ...
                {/* 결제 및 배송 정보 추가 */}
                <div>
                  <h3 className="text-lg font-medium mb-2 text-cyan-400">결제 및 배송 정보</h3>
                  <div className="text-sm space-y-1 bg-gray-800/50 p-3 rounded-lg border border-gray-800">
                    <p>
                      <span className="font-medium text-gray-300">배송 방법:</span> 
                      <span className="text-gray-200">
                        {selectedOrder.deliveryMethod === 'standard' ? '일반 배송' : 
                         selectedOrder.deliveryMethod === 'express' ? '빠른 배송' : 
                         selectedOrder.deliveryMethod === 'same-day' ? '당일 배송' : '일반 배송'}
                      </span>
                    </p>
                    <p>
                      <span className="font-medium text-gray-300">배송비:</span> 
                      <span className="text-gray-200">{new Intl.NumberFormat('ko-KR').format(selectedOrder.deliveryFee || 3000)}원</span>
                    </p>
                    <p>
                      <span className="font-medium text-gray-300">총 결제 금액:</span> 
                      <span className="text-green-400 font-semibold">{new Intl.NumberFormat('ko-KR').format(selectedOrder.amount + (selectedOrder.deliveryFee || 3000))}원</span>
                    </p>
                    {selectedOrder.paymentId && (
                      <p>
                        <span className="font-medium text-gray-300">결제 방법:</span> 
                        <span className="text-gray-200">온라인 결제</span>
                      </p>
                    )}
                    <p>
                      <span className="font-medium text-gray-300">결제 상태:</span> 
                      <span className={`${
                        (() => {
                          const paymentStatus = selectedOrder.paymentStatus || getPaymentStatusFromOrderStatus(selectedOrder.status || '결제완료');
                ...
                <div>
                  <h3 className="text-lg font-medium mb-2 text-cyan-400">고객 정보</h3>
                  <div className="text-sm space-y-1 bg-gray-800/50 p-3 rounded-lg border border-gray-800">
                    <p><span className="font-medium text-gray-300">이름:</span> <span className="text-gray-200">{selectedOrder.customerName}</span></p>
                    <p><span className="font-medium text-gray-300">이메일:</span> <span className="text-gray-200">{selectedOrder.customerEmail}</span></p>
                    {selectedOrder.customerPhone && (
                      <p><span className="font-medium text-gray-300">전화번호:</span> <span className="text-gray-200">{selectedOrder.customerPhone}</span></p>
                    )}
                    {selectedOrder.customerAddress ? (
                      <>
                        <p><span className="font-medium text-gray-300">배송 주소:</span> <span className="text-gray-200">{selectedOrder.customerAddress}</span></p>
                        {selectedOrder.customerZipCode && (
                          <p><span className="font-medium text-gray-300">우편번호:</span> <span className="text-gray-200">{selectedOrder.customerZipCode}</span></p>
                        )}
                      </>
                    ) : (
                      <p><span className="font-medium text-gray-300">배송 주소:</span> <span className="text-gray-400">배송 정보 없음</span></p>
                    )}
                  </div>
                </div>
                ...
                {selectedOrder.trackingNumber && (
                  <div>
                    <h3 className="text-lg font-medium mb-2 text-cyan-400">배송 정보</h3>
                    <div className="text-sm space-y-1 bg-gray-800/50 p-3 rounded-lg border border-gray-800">
                      <p>
                        <span className="font-medium text-gray-300">배송사:</span> 
                        <span className="text-gray-200">
                          {selectedOrder.shippingCompany === 'cj' ? 'CJ대한통운' : 
                           selectedOrder.shippingCompany === 'lotte' ? '롯데택배' :
                           selectedOrder.shippingCompany === 'hanjin' ? '한진택배' :
                           selectedOrder.shippingCompany === 'logen' ? '로젠택배' :
                           selectedOrder.shippingCompany === 'post' ? '우체국택배' : '택배사'}
                        </span>
                      </p>
                      <p>
                        <span className="font-medium text-gray-300">운송장번호:</span> <span className="text-gray-200">{selectedOrder.trackingNumber}</span>
                        <a 
                          href={getTrackingUrl(selectedOrder.shippingCompany || 'cj', selectedOrder.trackingNumber)}
                          target="_blank"
                          rel="noopener noreferrer"
                      ...
                      {selectedOrder.shippingNotified && selectedOrder.shippingNotifiedAt && (
                        <p><span className="font-medium text-gray-300">알림전송시간:</span> <span className="text-gray-200">{new Date(selectedOrder.shippingNotifiedAt).toLocaleString()}</span></p>
                      )}
                    </div>
                  </div>
                )}
                ...
                <div className="flex space-x-2">
                  ...
                  <Button
                    variant={selectedOrder.publishToGallery ? "destructive" : "outline"}
                    size="sm"
                    onClick={handlePublishToGallery}
                    disabled={isPublishing || (!selectedOrder.publishToGallery && !galleryTitle.trim())}
                    className={selectedOrder.publishToGallery 
                ...
                {!selectedOrder.publishToGallery && (
                  <div className="mt-3 space-y-2">
                    <Label htmlFor="gallery-title" className="text-gray-300">갤러리 표시 제목</Label>
                    <Input
                      id="gallery-title"
                      value={galleryTitle}
              ...
              <div>
                <h3 className="text-lg font-medium mb-2 text-cyan-400">라벨 디자인 미리보기</h3>
                    ...
                    {selectedOrder.labelImage ? (
                      <>
                        ...
                        <img 
                          src={(() => {
                            // 선택된 와인병 정보 가져오기
                            const bottle = selectedOrder.bottleId ? getWineBottle(selectedOrder.bottleId) : null;
                            return bottle?.image || "http://localhost:3000/images/wine-bottle-1.png";
                          })()}
                        ...
                        >
                          ...
                          <div className="relative">
                            ...
                            {/* 장식 렌더링 (장식 정보가 있는 경우) */}
                            {selectedOrder.labelDesign && selectedOrder.labelDesign.decorations && selectedOrder.labelDesign.decorations.length > 0 && 
                              selectedOrder.labelDesign.decorations.map((decoration: any, index: number) => (
                                <StaticElement
                                ...
                                >
                                  <img 
                                    src={decoration.image || decoration.decorationUrl || `http://localhost:3000/images/icon/${decoration.decorationId || 'default'}.png`}
                                    alt="장식"
