...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  if (!fs.existsSync(publicDir)) {
    ...
    console.log('[DEBUG] Created publicDir:', publicDir);
  } else {
    console.log('[DEBUG] publicDir exists:', publicDir);
  }

  // images 디렉토리 생성
  if (!fs.existsSync(imagesDir)) {
    fs.mkdirSync(imagesDir, { recursive: true });
    console.log('[DEBUG] Created imagesDir:', imagesDir);
  } else {
    console.log('[DEBUG] imagesDir exists:', imagesDir);
  }

  if (!fs.existsSync(labelDir)) {
    fs.mkdirSync(labelDir, { recursive: true });
    console.log('[DEBUG] Created labelDir:', labelDir);
  } else {
    console.log('[DEBUG] labelDir exists:', labelDir);
  }

  if (!fs.existsSync(iconDir)) {
    fs.mkdirSync(iconDir, { recursive: true });
    console.log('[DEBUG] Created iconDir:', iconDir);
  } else {
    console.log('[DEBUG] iconDir exists:', iconDir);
  }

  if (!fs.existsSync(borderDir)) { // 테두리 폴더 생성
    fs.mkdirSync(borderDir, { recursive: true });
    console.log('[DEBUG] Created borderDir:', borderDir);
  } else {
    console.log('[DEBUG] borderDir exists:', borderDir);
  }

  if (!fs.existsSync(uploadDir)) { // 업로드 폴더 생성
    fs.mkdirSync(uploadDir, { recursive: true });
    console.log('[DEBUG] Created uploadDir:', uploadDir);
  } else {
    console.log('[DEBUG] uploadDir exists:', uploadDir);
  }
  ...
  app.get("/api/admin/labels/icons", async (_req, res) => {
    try {
      console.log('[DEBUG] 아이콘 디렉토리 경로:', iconDir);
      console.log('[DEBUG] 아이콘 디렉토리 존재 여부:', fs.existsSync(iconDir));

      if (!fs.existsSync(iconDir)) {
        console.log('[DEBUG] 아이콘 디렉토리가 없어서 생성합니다.');
        fs.mkdirSync(iconDir, { recursive: true });
      }

      // 디렉토리에서 파일 목록 읽기
      const files = fs.readdirSync(iconDir);
      console.log('[DEBUG] 아이콘 디렉토리 파일 목록:', files);

      const icons = files.map(filename => {
        const id = path.parse(filename).name; // 확장자 제외한 파일명
        const fullPath = path.join(iconDir, filename);
        console.log('[DEBUG] 아이콘 파일 처리 중:', { filename, id, fullPath, exists: fs.existsSync(fullPath) });
      ...
      console.log('[DEBUG] 반환할 아이콘 목록:', icons.length, '개');
