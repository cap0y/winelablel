...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/users/change-password", async (req, res) => {
    try {
      ...
      if (!email || !currentPassword || !newPassword) {
        return res.status(400).json({ success: false, message: "이메일, 현재 비밀번호, 새 비밀번호가 필요합니다." });
      }
      ...
      console.error("[ERROR] 비밀번호 변경 오류:", error);
      return res.status(500).json({ success: false, message: "비밀번호 변경 중 오류가 발생했습니다: " + error.message });
    }
  });
  ...
  app.get("/api/payment/bank-config", async (_req, res) => {
    try {
      ...
      const accountNo = process.env.BANK_ACCOUNT || process.env.PAYMENT_BANK_ACCOUNT;
      const accountHolder = process.env.BANK_HOLDER || process.env.PAYMENT_BANK_HOLDER;

      if (!bankName || !accountNo || !accountHolder) {
        return res.status(400).json({
          success: false,
          message: "은행 계좌 정보가 설정되지 않았습니다. 환경변수를 설정하세요.",
        });
      }
  ...
  app.patch("/api/orders/:orderId/bank-payment", async (req, res) => {
    try {
      ...
      const { bankAccount, amount } = req.body as { bankAccount?: string; amount?: number };
