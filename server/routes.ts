...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/admin/labels/backgrounds", async (_req, res) => {
    try {
      ...
      const allCategoryRelations = await db
        ...
        .leftJoin(
          labelCategories,
          eq(labelBackgroundCategories.categoryId, labelCategories.id),
        );
      ...
      const backgrounds = files.map((filename) => {
        ...
        return {
          ...
          createdAt: fs
            .statSync(path.join(labelDir, filename))
      ...
      res.status(500).json({
        success: false,
        message:
          "라벨 배경 이미지 조회 중 오류가 발생했습니다: " + error.message,
      });
    }
  });
    ...
    try {
      ...
      const icons = files.map((filename) => {
        ...
        return {
          ...
          type: "icon",
          createdAt: fs
            .statSync(path.join(iconDir, filename))
            .birthtime.toISOString(),
        };
      });
      ...
      res.status(500).json({
        success: false,
        message:
          "아이콘/장식 이미지 조회 중 오류가 발생했습니다: " + error.message,
      });
    }
  });
    ...
    try {
      ...
      const borders = files.map((filename) => {
        ...
        return {
          ...
          type: "border",
          createdAt: fs
            .statSync(path.join(borderDir, filename))
            .birthtime.toISOString(),
        };
      });
  ...
  // 라벨 배경 이미지 업로드
  app.post(
    "/api/admin/labels/backgrounds/upload",
    uploadLabel.single("file"),
    async (req, res) => {
      try {
        if (!req.file) {
          return res.status(400).json({
        ...
        res.status(500).json({
          success: false,
          message:
            "라벨 배경 이미지 업로드 중 오류가 발생했습니다: " + error.message,
        });
      }
    },
  );
  ...
  // 테두리 이미지 업로드 API 추가
  app.post(
    "/api/admin/labels/borders/upload",
    uploadBorder.single("file"),
    async (req, res) => {
      try {
        if (!req.file) {
          return res.status(400).json({
        ...
        res.status(500).json({
          success: false,
          message:
            "테두리 이미지 업로드 중 오류가 발생했습니다: " + error.message,
        });
      }
    },
  );
  ...
  app.delete("/api/admin/labels/backgrounds/:filename", async (req, res) => {
    try {
      ...
      res.status(500).json({
        success: false,
        message:
          "라벨 배경 이미지 삭제 중 오류가 발생했습니다: " + error.message,
      });
    }
  });
  ...
  app.delete("/api/admin/labels/icons/:filename", async (req, res) => {
    try {
      ...
      res.status(500).json({
        success: false,
        message:
          "아이콘/장식 이미지 삭제 중 오류가 발생했습니다: " + error.message,
      });
    }
  });
  ...
  app.get("/api/admin/orders", async (req, res) => {
    try {
      ...
      console.log(
        `[INFO] 주문 목록 조회 시작 - 페이지: ${page}, 제한: ${limit}`,
      );
      ...
      console.log(
        `[INFO] 주문 목록 조회 완료 - ${endTime - startTime}ms, ${allOrders.length}건 조회`,
      );
  ...
  app.post("/api/admin/orders/:orderId/notify-shipping", async (req, res) => {
    try {
      ...
      const shippingCompanyName =
        shippingCompanyNames[shippingCompany] || "택배사";
      ...
      console.log(
        `[배송알림 내용] 주문번호: ${orderId}, 배송사: ${shippingCompanyName}, 운송장: ${trackingNumber}`,
      );
    ...
    try {
      ...
      try {
        ...
        if (paymentInfo.status === "PAID") {
          ...
          String(paymentInfo.status) === "CANCELLED" ||
          String(paymentInfo.status) === "FAILED"
        ) {
          // 결제 취소 또는 실패 상태
          await db
            .update(orders)
        ...
        return res.status(400).json({
          success: false,
          message:
            "결제 정보를 찾을 수 없습니다. 결제가 취소되었을 수 있습니다.",
          orderId,
        });
      }
    } catch (error: any) {
  ...
  app.get("/api/admin/stats/daily", async (req, res) => {
    try {
      ...
      const dailySales: Record<
        string,
        { date: string; sales: number; count: number }
      > = {};
  ...
  app.get("/api/admin/stats/monthly", async (req, res) => {
    try {
      ...
      const monthlySales: Record<
        string,
        { month: string; sales: number; count: number }
      > = {};
  ...
  app.get("/api/admin/stats/bottles", async (req, res) => {
    try {
      // 와인별 매출 계산
      const bottleSales: Record<
        string,
        { id: string; name: string; sales: number; count: number }
      > = {};
      ...
      // 매출 기준 내림차순 정렬
      const result = Object.values(bottleSales).sort(
        (a, b) => b.sales - a.sales,
      );
      ...
      res.status(500).json({
        success: false,
        message:
          "와인별 매출 통계 조회 중 오류가 발생했습니다: " + error.message,
      });
    }
  });
  ...
  app.get("/api/admin/stats/summary", async (req, res) => {
    try {
      ...
      const averageOrderValue = totalOrdersCount
        ? Math.round(totalSales / totalOrdersCount)
        : 0;
  ...
  app.get("/api/gallery/labels", async (req, res) => {
    try {
      ...
      labels.forEach((label, index) => {
        console.log(
          `[DEBUG] 라벨 ${index + 1}: ID=${label.id}, publishToGallery=${label.publishToGallery}, title=${label.title || "none"}`,
        );
      });
        ...
        labels.map(async (label) => {
          try {
            ...
            if (label.userId) {
              ...
              if (user.length > 0) {
                designerName =
                  user[0].displayName || user[0].username || "알 수 없음";
              }
            }
      ...
      res.status(500).json({
        success: false,
        message:
          "라벨 갤러리 목록 조회 중 오류가 발생했습니다: " + error.message,
      });
    }
  });
  ...
  app.get("/api/gallery/labels/popular", async (req, res) => {
    try {
      ...
      res.status(500).json({
        success: false,
        message:
          "인기 라벨 이미지 조회 중 오류가 발생했습니다: " + error.message,
      });
    }
  });
  ...
  app.get("/api/gallery/labels/:labelId", async (req, res) => {
    try {
      ...
      const label = await db
        .select()
        .from(orders)
        .where(and(eq(orders.id, labelId), eq(orders.publishToGallery, true)))
        .limit(1);
  ...
  app.post("/api/gallery/labels/:labelId/likes/toggle", async (req, res) => {
    try {
      ...
      const existingLike = await db
        ...
        .from(labelLikes)
        .where(
          and(eq(labelLikes.orderId, labelId), eq(labelLikes.userId, userId)),
        )
        .limit(1);
      ...
      if (existingLike && existingLike.length > 0) {
        ...
        // 좋아요 추가
        await db.insert(labelLikes).values({
          orderId: labelId,
          userId,
          createdAt: new Date(),
        });
  ...
  app.patch("/api/orders/:orderId/publish", async (req, res) => {
    try {
      ...
      res.status(500).json({
        success: false,
        message:
          "주문 갤러리 공개 설정 중 오류가 발생했습니다: " + error.message,
      });
    }
  });
    ...
    try {
      ...
      const uploads = files.map((filename) => {
        ...
        return {
          ...
          type: "upload",
          createdAt: fs
            .statSync(path.join(uploadDir, filename))
            .birthtime.toISOString(),
        };
      });
      ...
      res.status(500).json({
        success: false,
        message:
          "사용자 업로드 이미지 조회 중 오류가 발생했습니다: " + error.message,
      });
    }
  });
    ...
    async (req, res) => {
      try {
        ...
        res.status(500).json({
          success: false,
          message:
            "사용자 이미지 업로드 중 오류가 발생했습니다: " + error.message,
        });
      }
    },
  );
  ...
  app.delete("/api/labels/uploads/:filename", async (req, res) => {
    try {
      ...
      res.status(500).json({
        success: false,
        message:
          "사용자 업로드 이미지 삭제 중 오류가 발생했습니다: " + error.message,
      });
    }
  });
  ...
  app.post("/api/admin/labels/categories", async (req, res) => {
    try {
      ...
      res.status(500).json({
        success: false,
        message:
          "라벨 배경 카테고리 생성 중 오류가 발생했습니다: " + error.message,
      });
    }
  });
  ...
  app.patch("/api/admin/labels/categories/:categoryId", async (req, res) => {
    try {
      ...
      res.status(500).json({
        success: false,
        message:
          "라벨 배경 카테고리 수정 중 오류가 발생했습니다: " + error.message,
      });
    }
  });
  ...
  app.delete("/api/admin/labels/categories/:categoryId", async (req, res) => {
    try {
      ...
      res.status(500).json({
        success: false,
        message:
          "라벨 배경 카테고리 삭제 중 오류가 발생했습니다: " + error.message,
      });
    }
  });
  ...
  app.post(
    "/api/admin/labels/backgrounds/:backgroundId/categories",
    async (req, res) => {
      try {
        ...
        for (const categoryId of categoryIds) {
          ...
          if (categoryExists.length) {
            const relation = await db
              .insert(labelBackgroundCategories)
              .values({
                backgroundId,
  ...
  app.get(
    "/api/labels/backgrounds/category/:categorySlug",
    async (req, res) => {
      try {
        ...
        const backgroundRelations = await db
          .select({
            backgroundId: labelBackgroundCategories.backgroundId,
          })
          .from(labelBackgroundCategories)
        ...
        const backgroundIds = backgroundRelations.map(
          (relation) => relation.backgroundId,
        );
        ...
        res.status(500).json({
          success: false,
          message:
            "카테고리별 배경 이미지 목록 조회 중 오류가 발생했습니다: " +
            error.message,
        });
      }
    },
  ...
  app.get("/api/labels/categories", async (_req, res) => {
    try {
      ...
      res.status(500).json({
        success: false,
        message:
          "라벨 카테고리 목록 조회 중 오류가 발생했습니다: " + error.message,
      });
    }
  });
  ...
  app.get("/api/admin/bottles", async (_req, res) => {
    try {
      ...
      res
        ...
        .json({
          success: false,
          message: "와인병 목록을 불러오는데 실패했습니다: " + error.message,
        });
    }
  });
  ...
  app.get("/api/admin/bottles/:bottleId", async (req, res) => {
    try {
      ...
      if (bottle.length === 0) {
        return res
          .status(404)
          .json({ success: false, message: "해당 와인병을 찾을 수 없습니다." });
      }
      ...
      res
        ...
        .json({
          success: false,
          message: "와인병을 불러오는데 실패했습니다: " + error.message,
        });
    }
  });
  ...
  app.post("/api/admin/bottles", async (req, res) => {
    try {
      ...
      res
        ...
        .json({
          success: false,
          message: "와인병 추가 중 오류가 발생했습니다: " + error.message,
        });
    }
  });
  ...
  app.put("/api/admin/bottles/:bottleId", async (req, res) => {
    try {
      ...
      // 해당 와인병이 존재하는지 확인
      const existingBottle = await db
        .select({ id: wineBottles.id, image: wineBottles.image })
        .from(wineBottles)
        .where(eq(wineBottles.id, bottleId))
        .limit(1);
      ...
      res.json({
        success: true,
        message: "와인병 정보가 성공적으로 업데이트되었습니다.",
      });
    } catch (error: any) {
      ...
      res
        ...
        .json({
          success: false,
          message: "와인병 수정 중 오류가 발생했습니다: " + error.message,
        });
    }
  });
  ...
  app.delete("/api/admin/bottles/:bottleId", async (req, res) => {
    try {
      ...
      res
        ...
        .json({
          success: false,
          message: "와인병 삭제 중 오류가 발생했습니다: " + error.message,
        });
    }
  });
  ...
  // 와인병 이미지 업로드 API
  app.post(
    "/api/admin/bottles/upload",
    bottleUpload.single("file"),
    async (req, res) => {
      try {
        if (!req.file) {
          return res
        ...
        res.status(500).json({
          success: false,
          message:
            "와인병 이미지 업로드 중 오류가 발생했습니다: " + error.message,
        });
      }
    },
  );
